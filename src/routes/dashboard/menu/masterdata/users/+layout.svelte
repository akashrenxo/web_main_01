<script lang="ts">
    ...
    import ShowUserRolesModal from "$lib/components/dashboard/user/modal/userRolesModal.svelte";
    import Loader from "$lib/components/dashboard/loader/loader.svelte";
        ...
        <div class="w-full max-w-7xl mx-auto px-4 py-8 z-0 font-sans">
            ...
            <div class="relative h-[450px] rounded-lg mt-5">
                <div class="overflow-auto h-full scrollbar-beautiful">
                    {#if $userAttributes && $userAttributes.length > 0}
                        <table
                            class="table-auto w-full text-sm border text-gray-700 bg-white relative"
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            {#if showEditMenu && activeMenuUserId === user["user"]}
                                                <div
                                                    bind:this={menuNode}
                                                    class="absolute top-{$offsetFromTop} right-24 mt-2 rounded-md bg-white shadow-lg z-[4] origin-top-right"
                                                    in:fade={{ duration: 200 }}
                                                    out:fade={{ duration: 200 }}
                                                    style="filter: drop-shadow(0 0 10px rgba(0,0,0,0.1));"
                                                >
                                                    <div
                                                        class="py-1 min-w-[160px]"
                                                    >
                                                        <button
...
{#if showUserRolesModal}{/if}

{#if showUserEditModal}
    <UserEditModal
        bind:showUserEditModal
        {selectedUserName}
        bind:activeMenuUserId
    />
{/if}
